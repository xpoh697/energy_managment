            ...
            if is_before_limit_hour and has_surplus and not (is_throttled or is_energy_low_for_evening) and cur_price > 0:
                ...
                if is_throttled or is_energy_low_for_evening:
                    if is_throttled:
                         ...
                         reason = f"Продажа до {sell_strategy.get('morning_autopilot_floor')}% (защита утра)"
                    ...
                    reason = f"Цена ({cur_price or 0.0:.2f}) >= Порога ({price_sell_only_pv or 0.0:.2f})"
                elif not has_surplus:
                    reason = f"Цена ({cur_price or 0.0:.2f}) >= Порога ({price_sell_only_pv or 0.0:.2f}), но нет излишка солнца"
                else:
